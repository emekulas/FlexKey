...
void setup() {
    ...
    // Generate device ID from ESP32 Chip ID (more reliable than MAC)
    if (systemConfig.deviceID.length() == 0) {
        ...
        systemConfig.deviceID = String(AP_MODE_SSID_PREFIX) + String(chipIdStr).substring(6); // Last 6 chars
        systemConfig.deviceID.toUpperCase();
        storage.saveDeviceID(systemConfig.deviceID);
        Serial.println("[SYSTEM] Generated Device ID from Chip ID");
    }
